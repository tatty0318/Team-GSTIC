...
export class PersonaEmpleadoController {
  ...
  @get('/personas/{id}/empleado', {
    responses: {
      '200': {
        description: 'Persona has one Empleado',
        content: {
          'application/json': {
  ...
  async get(
    ...
    return this.personaRepository.empleado(id).get(filter);
  }

  @post('/personas/{id}/empleado', {
    responses: {
      '200': {
  ...
  async create(
    ...
    return this.personaRepository.empleado(id).create(empleado);
  }

  @patch('/personas/{id}/empleado', {
    responses: {
      '200': {
  ...
  async patch(
    ...
    return this.personaRepository.empleado(id).patch(empleado, where);
  }

  @del('/personas/{id}/empleado', {
    responses: {
      '200': {
  ...
  async delete(
    ...
    return this.personaRepository.empleado(id).delete(where);
  }
}
